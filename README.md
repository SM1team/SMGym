...
|:--:|--|--|
|곽인성|⦁ 문제점1: QR 코드가 카메라에 명확히 비춰졌음에도 불구하고, "QR SUCCESS" 메시지가 표시되지 않음.<br><br>⦁ 문제점2: 로드밸런서 생성 후 클라이언트에서 요청 시에 트래픽이 안잡히는 현상, 로드밸런서 포트로 접근 시 페이지 로딩 에러 발생<br><br>⦁ 문제점3: 팝업창 실행시 JQuery에서 Uncaught ReferenceError : functionName is not defined 에러 발생|⦁ 솔루션1: ### **해결 방법**
